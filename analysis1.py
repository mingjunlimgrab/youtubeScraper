...
'grabexpress':5, 'grabpay':5, 'justgrab':5, 'grab':1.5, 'uber':4, 'GOJEK': 4, 'mod': 3, 'driver': 4, 'drivers': 4, 'app': 3, 'car': 3, 'taxi': 4, 'job': 4, 'version': 2, 'delivery': 2,
'private': 1, 'hire': 2, 'singapore': 3, 'sg':3, 'malaysia':3, 'vietnam':2, 'vn': 2, 'phillippines': 2, 'ph': 2, 'kuala':1, 'lumpur':1, 'promo':3, 'premium': 2,
...
def algorithm(args):
    ...
    if len(args)==1:
        val = args[0] * 1.5
    if len(args)==2:
        val = args[0] * 1.5 + args[1] * 1.3
    if len(args)==3:
        val = args[0] * 1.5 + args[1] * 1.3 + args[2] * 1.2
    if len(args)==4:
        val = args[0] * 1.5 + args[1] * 1.3 + args[2] * 1.2 + args[3] * 1.1
    if len(args)>=5:
        val = args[0] * 1.5 + args[1] * 1.3 + args[2] * 1.2 + args[3] * 1.1 + args[4] * 1.0
    if val >= 10:
        return 10
    else:
        return val
