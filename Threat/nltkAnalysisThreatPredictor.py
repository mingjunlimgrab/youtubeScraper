...
def spitter(strink):
    ...
    if checker(title.lower()):
        ...
        for item in toke:
            cleaned.append(item)
        d_f = document_features(cleaned)
        spit = LinearSVC_classifier.classify(d_f)
    return spit
...
def outputSaver(NBclassifierFn, csvTest):
    ...
    for item in df['title']:
        titlez = dehypdeslash(item)
        toke = special_features(titlez)
        cleaned = clean(titlez)
        for item in toke:
            cleaned.append(item)
        d_f = document_features(cleaned)
        if NBclassifierFn(d_f) == 'HIG':
            ...
            pred_relevance.append(df['pred_relevance'][index])
            pred_threat.append('HIG')
        index += 1
    ...
    new_df['pred_threat'] = pred_threat

    new_df.to_csv('/Users/mingjun.lim/Documents/youtubeScraper/Data/secondRound.csv')
...
outputSaver(LinearSVC_classifier.classify, '/Users/mingjun.lim/Documents/youtubeScraper/Data/firstRound.csv')
